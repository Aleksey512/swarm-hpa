...
func TestRouterDispatch(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ds, pm := &recordProvider{val: 1}, &recordProvider{val: 2}
			r := NewRouter(ds, pm, tc.defaultSource, discardLogger())
...
func TestRouterPrometheusUnconfigured(t *testing.T) {
	r := NewRouter(&recordProvider{}, nil, config.ProviderDockerStats, discardLogger())
...
func TestRouterUnknownSource(t *testing.T) {
	r := NewRouter(&recordProvider{}, &recordProvider{}, config.ProviderDockerStats, discardLogger())
...
func TestRouterPropagatesProviderError(t *testing.T) {
	...
	r := NewRouter(ds, nil, config.ProviderDockerStats, discardLogger())
